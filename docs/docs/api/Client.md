...
* **webSocket** `WebSocketOptions` (optional) - WebSocket-specific configuration options.
  * **maxDecompressedMessageSize** `number` (optional) - Default: `67108864` (64 MB) - Maximum allowed size in bytes for decompressed WebSocket messages when using the permessage-deflate extension. Protects against decompression bomb attacks where a small compressed payload expands to an extremely large size. The check uses a conservative 10x expansion ratio estimate for early rejection. Set to 0 to disable the limit.
